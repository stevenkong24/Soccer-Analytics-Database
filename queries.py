...
def Q_2(cursor, conn, execution_time):
    ...
    query = """SELECT player_name, COUNT(*)
    FROM events INNER JOIN matches ON events.match_id = matches.match_id
...
def Q_5(cursor, conn, execution_time):
    ...
    query = """SELECT recipient_name, count(*) 
    FROM events INNER JOIN matches ON events.match_id = matches.match_id
...
def Q_6(cursor, conn, execution_time):
    ...
    query = """ SELECT team_name, COUNT(*)
    FROM events INNER JOIN matches ON events.match_id = matches.match_id
    WHERE competition_name = 'Premier League' AND season_name = '2003/2004' AND events.type_name = 'Shot'
    GROUP BY team_name
    ORDER BY COUNT(*) DESC"""
